...
class PriceLevelsProcessor(IndicatorProcessor):
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(len(df)):
            ...
            if len(current_week_idx) > 0 and current_week_idx[0] > 0:
                ...
                if prev_week_id in weekly_stats:
                    ...
                    df.loc[df.index[i], 'is_low_above_last_week_high'] = current_row['low'] > prev_week_high
                    df.loc[df.index[i], 'is_high_below_last_week_low'] = current_row['high'] < prev_week_low
        ...
        df['last_2_inside_bar_date'] = df['last_2_inside_bar_date'].ffill()
